...
**`.env`를 다루는 모든 행동은 「보안상 위험이 없다」가 먼저 성립해야 시작합니다.**
편의·속도·디버깅 어느 것도 이 전제보다 앞서지 않습니다. 애매하면 하지 말고 물어보세요.

| 하지 말 것 | 대신 |
|---|---|
| `cat .env` · `head .env` — 값이 화면·로그·대화에 남습니다 | 키 이름만: `grep -o '^[A-Z_]*=' .env` |
| `sed`로 가려서 출력 — **짧은 값은 그대로 새어 나갑니다** | 가리느니 찍지 않습니다 |
| 값을 커밋·붙여넣기·업로드 | 셸 변수로만: `set -a; . ./.env; set +a` 뒤 `"$VAR"` |
| `.env.example`에 실값 | 자리표시자(`<발급받은-키>`) |
| 파일을 지우거나 덮어쓰기 — 되돌릴 수 없습니다 | 옮기는 것만, 그것도 사용자에게 알리고 |

만료된 토큰은 **되살리기 전에 왜 죽었는지 먼저 봅니다.** 장기 토큰을 새로 발급받는 것보다
그 줄을 주석 처리하고 정상 로그인 경로를 쓰는 쪽이 대체로 안전합니다.
...
4. **도구 호출을 강제하지 않으면 파싱이 통째로 실패합니다** — `strict: true`는 도구가 **호출됐을 때** 입력 스키마만 검증하고, 호출 자체는 보장하지 않습니다. 기본값 `tool_choice: {"type":"auto"}`면 모델이 도구 없이 텍스트만 반환할 수 있습니다. 도구를 쓰는 경로(조건 파싱·판별)는 **`tool_choice: {"type":"tool","name":"..."}`로 강제**하세요.
   ...
   - ⚠️ **Fable 5.1 계열만 예외입니다** — 그 모델은 `tool_choice`의 `tool`·`any`를 **400으로 거부**합니다. 거기서는 `auto` + 도구 이름을 명시한 지시문 + `strict: true`로 대신합니다. 우리가 쓰는 `claude-sonnet-5`는 해당 없습니다.
